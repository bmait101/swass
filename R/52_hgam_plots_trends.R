...
## Plot for DNR Brook Trout Reserves Program ===============================

p.data <- bind_rows(
  pred.bkt0 %>% mutate(group = "a", species="Brook Trout", age="Age-0"),
  pred.bkt1 %>% mutate(group = "a", species="Brook Trout", age="Age-1+"), 
  pred.bnt0 %>% mutate(group = "b", species="Brown Trout", age="Age-0"), 
  pred.bnt1 %>% mutate(group = "b", species="Brown Trout", age="Age-1+")
) %>% 
  mutate(year_f = 2010) %>% 
  relocate(year_f, .after = reach_id) %>% 
  bind_rows(
    pred.bkt0.y2y %>% mutate(group = "c", species="Brook Trout", age="Age-0"),
    pred.bkt1.y2y %>% mutate(group = "c", species="Brook Trout", age="Age-1+"), 
    pred.bnt0.y2y %>% mutate(group = "d", species="Brown Trout", age="Age-0"), 
    pred.bnt1.y2y %>% mutate(group = "d", species="Brown Trout", age="Age-1+")
  ) %>% 
  mutate(
    age = factor(age, levels = c("Age-0", "Age-1+")), 
    group = factor(group, levels = c("a","b","c","d")))
...
p.dnr <- p.data %>% 
  ...
  labs(
    ...
    y = "Relative abundance (fish per mile)"
  ) + 
  theme_minimal(base_family = "sans", base_size = 12) +
  theme(
    plot.background = element_rect(fill = "white", color = NA),
...
path <- here::here("output", "figs1", "dnr_bkt_reserves_trends")
ggsave(glue::glue("{path}.pdf"), plot = p.dnr, 
       width = 9, height = 6, device = cairo_pdf)
pdftools::pdf_convert(pdf = glue::glue("{path}.pdf"),
                      filenames = glue::glue("{path}.png"),
                      format = "png", dpi = 600)
